...
un_list = ('deepika','binod','bhatta')
pw_list = ('12345','promax','khatraa')

def login():
    un = textbox_UN.get()
    pw = textbox_PASS.get()
    
    if un_list.count(un) == 1:
        indexUN = un_list.index(un)
        if pw_list[indexUN] == pw:
            messagebox.showinfo('Login Success', f'Welcome {un}!')
        else:
